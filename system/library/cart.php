...
class Cart {
    ...
    public function getProducts(){
        if (!$this->data) {
            ...
            foreach ($this->session->data['cart'] as $key => $quantity) {
                ...
                if (!empty($product[3])) { //$product[2] Changed for grp_id
                    $profile_id = $product[3]; //$product[2]
                } else {
                    $profile_id = 0;
                }
                ...
                if ($product_query->num_rows) {
                    ...
                    //print_r($this->session->data['groupd_id']);die;
                    if ($grp_id != '') {
                        //$grpproduct_id = $grp_id;
                        $grpproduct_id = $product[3];
                        foreach ($options as $product_option_id => $option_value) {
                            $option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int) $product_option_id . "' AND po.product_id = '" . (int) $grpproduct_id . "' AND od.language_id = '" . (int) $this->config->get('config_language_id') . "'");
                    ...
                    $this->data[$product[3]][implode(":", array_column($option_data, 'product_option_value_id'))][] = array(
                        'key' => $key,
    ...
    public function getRecurringProducts() {
        ...
        foreach ($this->getProducts() as $main_product) {
