...
class ControllerProductProduct extends Controller
{
    ...
    public function index() {
        ...
        if (isset($this->request->get['product_id'])) {
            ...
            //if(isset($this->session->data['groupd_id'])){
            //unset($this->session->data['groupd_id']);
            //}
            //echo '<pre>';print_r($this->session->data['groupd_id']);die;
        } else {
            $product_id = 0;
        }
        ...
        if ($product_info) {
          ...
          $this->document->setTitle($product_info['name']);
            ...
            foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
                ...
                } elseif ($option['type'] == 'text' || $option['type'] == 'textarea' || $option['type'] == 'file' || $option['type'] == 'date' || $option['type'] == 'datetime' || $option['type']
                    == 'time') {
                    $this->data['options'][] = array(
                            'product_option_id' => $option['product_option_id'],
                            'option_id'         => $option['option_id'],
    ...
    public function testimonials()
    {
        ...
        $this->data['reviews'] = array();
        $review_total = $this->model_catalog_review->getTotalReviewstestimonial($this->request->get);
        $results = $this->model_catalog_review->getTestmonials($this->request->get);

              foreach ($results as $result) {
                      $this->data['reviews'][] = array(
                    'author'     => $result['customers_name'],
                    'text'       => $result['review_text'],
                    'rating'     => (int) $result['review_rate'],
                    'locations'  => $result['review_location'],
                    'pname'      => $result['products_name'],
                    'reviews'    => sprintf($this->language->get('text_reviews'), (int) $review_total),
                    'date_added' => date($this->language->get('date_format_short'), strtotime($result['review_date']))
            );
        }
        ...
        $pagination->limit = $this->config->get('config_admin_limit');
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = $this->url->link('product/product/testimonials', "&page={page}");

        $this->data['pagination'] = $pagination->render();

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/testimonials.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/product/testimonials.tpl';
        } else {
            $this->template = 'default/template/product/testimonials.tpl';
        }
        $this->children = array(
                'common/column_left',
                'common/column_right',
