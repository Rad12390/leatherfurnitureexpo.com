...
class ControllerCheckoutCartCustom extends Controller {
    ...
    public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            'href' => $this->url->link('checkout/cart_custom'),
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $this->data['text_button_continue'] = $this->language->get('text_button_continue');
            $this->data['text_button_signup'] = $this->language->get('text_button_signup');
            $this->data['text_button_login'] = $this->language->get('text_button_login');
            ...
            $this->data['action'] = $this->url->link('checkout/cart_custom');
            ...
            foreach ($products as $product) {
              ...
              $href=  $this->url->link('product/product', 'product_id=' . $product['product_id']);
                $canonical_settings = $this->config->get('canonical_module');
                 if(isset($canonical_settings[0]['status']) && $canonical_settings[0]['status']==1) { 
                          $path = '';
                          $this->load->model('custom/custom'); 
                          $default_category_id =  $this->model_custom_custom->get_default_category($product['product_id']); 
                          if($default_category_id > 0){
                           $path =  $this->model_custom_custom->get_rute_categories($default_category_id); 
                   }
                           if(!empty($path)){
                         $href =  $this->url->link('product/product', 'path=' . $path . '&product_id=' . $product['product_id'] . $url);
                           }
                }
                $this->data['products'][] = array(
                    'key' => $product['key'],
                    ...
                    'total' => $total,
                    'href' => $href,
                    'remove' => $this->url->link('checkout/cart_custom', 'remove=' . $product['key']),
            ...
            if (!empty($this->session->data['vouchers'])) {
                foreach ($this->session->data['vouchers'] as $key => $voucher) {
                    $this->data['vouchers'][] = array(
                        ...
                        'remove' => $this->url->link('checkout/cart_custom', 'remove=' . $key)
                    );
                }
            }
            ...
            $this->data['checkout'] = $this->url->link('checkout/cart_custom_two', '', 'SSL');

            $this->load->model('setting/extension');

            $this->data['checkout_buttons'] = array();

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/cart_custom.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/checkout/cart_custom.tpl';
            } else {
                $this->template = 'default/template/checkout/cart_custom.tpl';
            }
