...
//echo date_default_timezone_get();
        ...
        if (isset($this->request->post['coupon']) && $this->validateCoupon()) {
            ...
            $coupon_details = $this->model_checkout_coupon->getCoupon($this->request->post['coupon']);
            $coupon_details_time = strtotime($coupon_details['date_expire']);
            $coupon_details['date_expire'] = date('Y/m/d', $coupon_details_time);

            $date = date("Y/m/d ");

            if ($coupon_details['date_expire'] >= $date) {
                $this->session->data['couponexpiredate'] = $coupon_details['date_expire'];
            }

            $this->session->data['coupon'] = $this->request->post['coupon'];

            $this->session->data['coupon_name'] = $coupon_details['name'];
        ...
        $this->document->addScript('catalog/view/javascript/jquery_countdown/js/jquery.plugin.min.js');
        $this->document->addScript('catalog/view/javascript/jquery_countdown/js/jquery.countdown.js');
        ...
        if (isset($this->session->data['coupon']) && $this->session->data['couponexpiredate']) {
            $this->data['couponexpiredate'] = $this->session->data['couponexpiredate'];
        }
 ...
 $this->load->model('catalog/product');
            ...
            foreach($subproducts_list as $product){
                //echo count($subproducts_list);    
                 $product_name = $this->model_catalog_product->specialcategory($product['main_product_name']);
                        foreach ($product_name as $cat_id) {
                          
                            if ($cat_id['category_id'] == 59 || $cat_id['category_id'] == 86) {
                               $this->data['stressless_text'] = $this->language->get('text_stressless');
                        }}
