...
class ControllerCheckoutCustomValidation extends Controller {
    ...
    public function signUpValidate() {
        ...
        if (isset($this->session->data['register_account']['email'])) {
            unset($this->session->data['register_account']['email']);
        }
        if (isset($this->session->data['register_account']['password'])) {
            unset($this->session->data['register_account']['password']);
        }
        if (isset($this->session->data['register_account']['password'])) {
            unset($this->session->data['register_account']['confirm']);
        }
        $json = array();
    ...
    public function registerValidate() {
        ...
        if (!$json) {
            if ($this->customer->isLogged()) {
                ...
                // if user is logged in  and user has new address  
                ...
                if (!$json) {
                 ...
                 else if ($this->request->post['payment_method'] == 'greensky') {
                          $account_no = $this->request->post['account_no'];
                          
                           if ((utf8_strlen($this->request->post['account_no'])== " ") || (utf8_strlen($this->request->post['account_no']) < 16)) {
                            $json['error']['account_no'] = 'Account number is Invalid!';
                        } else {
                            $greensky_card_detail['account_no'] = $this->request->post['account_no'];
                        }
                        
//                         if ((utf8_strlen($this->request->post['greensky_cvv']) < 3) || (utf8_strlen($this->request->post['greensky_cvv']) > 4)) {
//                            $json['error']['greensky_cvv'] = ' Cvv number is Invalid!';
//                        } else {
//                            $greensky_card_detail['greensky_cvv'] = $this->request->post['greensky_cvv'];
//                        }
//                        if ((utf8_strlen($this->request->post['card_expiry_month']) < 1)) {
//                            $json['error']['greensky_validity'] = 'Validity required';
//                        } else {$greensky_card_detail
//                            $greensky_card_detail['greensky_card_expiry_month'] = $this->request->post['greensky_card_expiry_month'];
//                        }
//                        if ((utf8_strlen($this->request->post['greensky_card_expiry_year']) < 1)) {
//                            $json['error']['greensky_validity'] = 'Validity required';
//                        } else {
//                            $greensky_card_detail['greensky_card_expiry_year'] = $this->request->post['greensky_card_expiry_year'];
//                        }
//                         if (!($json['error']['greensky_validity'])) {
//                            if ((strtotime(date($this->request->post['greensky_card_expiry_year'] . "/" . $this->request->post['greensky_card_expiry_month'] . "/01"))) < (strtotime(date('Y/m/01')))) {
//                                $json['error']['greensky_validity'] = 'Card is  expired!';
//                            }
//                        }
                        ...
                        $greensky_card_detail['greensky_card_expiry_year'] = $this->request->post['greensky_card_expiry_year'];
                          ...
                          $this->session->data['greensky_card_detail'] = $greensky_card_detail; 
                     }
                ...
                if (!$json) {
                    ...
                    if (!$json['error']['email'] && (utf8_strlen($this->request->post['email']) > 1) && $this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])) {
                        $json['error']['warning'] = $this->language->get('error_exists');
                        $json['error']['warning'] .= sprintf($this->language->get('entry_login'), $this->url->link('account/login', '', 'SSL'));
                    }
...
//                            $greensky_card_detail['greensky_cvv'] = $this->request->post['greensky_cvv'];
//                        }
//                        if ((utf8_strlen($this->request->post['greensky_card_expiry_month']) < 1)) {
//                            $json['error']['greensky_validity'] = 'Validity required';
//                        } else {
//                            $greensky_card_detail['greensky_card_expiry_month'] = $this->request->post['greensky_card_expiry_month'];
//                        }
...
//                        }
                ...
                if (!$json) {
                    if (isset($this->request->post['password'])) {
                        ...
                        $shipping_address['country_id'] = $this->request->post['shipping_country_id'];
                        $shipping_address['zone_id'] = $this->request->post['shipping_zone_id'];
                    } elseif (!$this->config->get('config_guest_checkout') || $this->config->get('config_customer_price') || $this->cart->hasDownload()) {
