...
<form action="<?php echo $action; ?>" method="post">
    ...
    <input type="hidden" name="item_number_<?php echo $i; ?>" value="<?php echo $detail['model']; ?>" />
    <input type="hidden" name="amount_<?php echo $i; ?>" value="<?php echo $detail['price']; ?>" />
    <input type="hidden" name="quantity_<?php echo $i; ?>" value="<?php echo $detail['quantity']; ?>" />
    <?php $j = 0; ?>
    <?php foreach ($product['option'] as $option) { ?>
    <input type="hidden" name="on<?php echo $j; ?>_<?php echo $i; ?>" value="<?php if($option['name'] == 'Select A Grade') { echo 'Grade'; } elseif($option['name'] == 'Select A Color') { echo 'Color'; } else { echo $option['name']; }  ?>" />
    <input type="hidden" name="os<?php echo $j; ?>_<?php echo $i; ?>" value="<?php echo $option['option_value']; ?>" />           
    <?php $j++; ?>   
    <?php } ?>
    <?php $i++;  } ?>
    <?php } ?>  
