    ...
    <div class="content">
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="form">
          ...
          <td>
          <input type="text" name="review_date" value=" <?php echo $date; ?>" class="date"/>
          </td>
         </tr>
          <tr>
...
$('input[name=\'product\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/product_grouped/product_grouped_autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent( '%'+request.term),  // pass % before filter name
			dataType: 'json',
