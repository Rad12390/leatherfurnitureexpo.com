     ...
     $html .= '<option value="'. $value['code'] .'"  selected="selected" >'. $value['title'] .'</option>';
      ...
      $html .= '<option value="'. $value['code'] .'">'. $value['title'] .'</option>';
                    ...
                    <select name="custom_payment_method" style="display:none">
                        <option value="credit" <?php if($custom_payment_method == 'credit') echo 'selected'; ?>>Credit Card</option>
                        <option value="paypal" <?php if($custom_payment_method == 'paypal') echo 'selected'; ?>>Paypal</option>
                        <option value="affirm" <?php if($custom_payment_method == 'affirm') echo 'selected'; ?>>Affirm</option>
                        </select>
            ...
            <tr class="credit_card_fields">
              <td class="left"><span class="required">*</span>Credit Card Type </td>
              <td class="left">
                  <select name="credit_card_type" class="credit">
                        <option value="visa" <?php if($credit_card_type == 'visa') echo 'selected'; ?>>Visa</option>
                        <option value="visa-debit" <?php if($credit_card_type == 'visa-debit') echo 'selected'; ?>>Visa Debit</option>
                        <option value="mastercard" <?php if($credit_card_type == 'mastercard') echo 'selected'; ?>>MasterCard</option>
                        <option value="mastercard-debit" <?php if($credit_card_type == 'mastercard-debit') echo 'selected'; ?>>MasterCard Debit</option>
                        <option value="discover" <?php if($credit_card_type == 'discover') echo 'selected'; ?>>Discover</option>
                        <option value="american-express" <?php if($credit_card_type == 'american-express') echo 'selected'; ?>>American Express</option>
                        </select>
                <?php if ($error_credit_card_type) { ?>
                <span class="error"><?php echo $error_credit_card_type; ?></span>
                <?php } ?></td>
            </tr>
            <tr class="credit_card_fields">
              <td class="left"><span class="required">*</span>Credit Card Number </td>
              <td class="left"><input type="text" name="credit_card_number" value="<?php echo $credit_card_number;?>" />
                  <input type="hidden" name="credit_card_mask" value="<?php if(!$order_id) { echo '-1'; } ?>"/>
                  <?php if($credit_card_number) { ?><a href="javascript:void(0)" class="credit_card_mask" style=" text-decoration:none; padding: 0 0 0 5px; font-size:16px;"><span style=" display:inline-block; padding:2px 0 0 ;"> Mask this number </span></a><?php } ?>
                <?php if ($error_credit_card_number) { ?>
                <span class="error"><?php echo $error_credit_card_number; ?></span>
                <?php } ?>   
              </td>
            </tr>
            <tr class="credit_card_fields">
              <td class="left"><span class="required">*</span>Card Verification Value (CVV) </td>
              <td class="left"><input type="text" name="card_verification_value" value="<?php echo $card_verification_value;?>" />
                <?php if ($error_card_verification_value) { ?>
                <span class="error"><?php echo $error_card_verification_value; ?></span>
                <?php } ?></td>
            </tr>
            <tr class="credit_card_fields">
              <td class="left">Expires </td>
                <td class="left">
                  <!--<input type="text" name="payment_firstname" value="<?php  echo sprintf('%02d', $card_expiry_month) . '/'.$card_expiry_year ; ?>" />-->
                  <?php $months = array(
                    ...
                    <select name="card_expiry_month">
                        ...
                        <option value="<?php echo $key_month; ?>" <?php if($key_month == $card_expiry_month) echo 'selected'; ?>><?php echo $month; ?></option>
                        <?php } ?>
                    </select>
                    <select name="card_expiry_year">
                                <?php for($i = date("Y") ; $i < date("Y")+16; $i++ ) { ?>
                                <option value="<?php echo $i; ?>" <?php if($i == $card_expiry_year) echo 'selected'; ?>><?php echo $i; ?></option>
                                <?php } ?>
                    </select>
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    $('.credit_card_mask').bind('click', function() {
        $('input[name=\'credit_card_number\']').val('');;
        $('input[name=\'card_verification_value\']').val('');  
        $('input[name=\'credit_card_mask\']').val('1');  
        $('.credit_card_mask').remove();
});

$('select[name="custom_payment_method"]').bind('change', function() {
    if($('select[name=\'payment\']').val()) {
        if($(this).val() == 'credit') {
           $('.credit_card_fields').removeClass('dpn');
        } else {  
             $('.credit_card_fields').addClass('dpn');
        }
     } else {
         $('.credit_card_fields').addClass('dpn');
     }
