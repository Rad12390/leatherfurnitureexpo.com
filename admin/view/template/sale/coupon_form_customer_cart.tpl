...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      ...
      <div class="buttons">
          <?php if ($coupon_id && $history_total) { ?>
          <!--a href="<?php echo $export; ?>" class="button">Export</a-->
          <?php } ?>
          <a onclick="sendParentValue();" class="button"><?php echo $button_save; ?></a>
        ...
        <div id="tab-general">
          <table class="form">
            ...
            <tr>
              ...
              <td><input type="text" name="code" id="code" value="<?php echo $code; ?>" />
                <?php if ($error_code) { ?>
            ...
            <tr>
              <td><?php echo "Exclude Category" ?></td>
              <td><input type="text" name="exclude-category" value="" /></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td><div id="coupon-exclude-category" class="scrollbox">
                      <?php //echo "<pre>"; print_r($coupon_exclude_category); echo "</pre>"; ?>
                      <?php $class = 'odd'; ?>
                  <?php foreach ($coupon_exclude_category as $coupon_exclude_category) { ?>
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                  <div id="coupon-exclude-category<?php echo $coupon_exclude_category['category_id']; ?>" class="<?php echo $class; ?>"> 
                      <?php echo $coupon_exclude_category['name']; ?>
                      <img src="view/image/delete.png" alt="" />
                    <input type="hidden" name="coupon_exclude_category[]" value="<?php echo $coupon_exclude_category['category_id']; ?>" />
                  </div>
                  <?php } ?>
...
$('input[name=\'exclude-category\']').autocomplete({  //alert('second');
	delay: 500,
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
	...
	select: function(event, ui) {
		$('#coupon-exclude-category' + ui.item.value).remove();
		
		$('#coupon-exclude-category').append('<div id="product-category' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="coupon_exclude_category[]" value="' + ui.item.value + '" /></div>');

		$('#coupon-exclude-category div:odd').attr('class', 'odd');
		$('#coupon-exclude-category div:even').attr('class', 'even');
...
$('#coupon-exclude-category div img').live('click', function() {
	$(this).parent().remove();
...
function sendParentValue()
{
    $('input[name=\'name\']').next("span").remove();
    $('input[name=\'code\']').next("span").remove();
    ...
    if(a == '')
    {
        ...
        return false;
    } else { $('input[name=\'name\']').css('border', '1px solid #ccc'); }
    
    if(a.length < 3 || a.length > 128)
    {
        $('input[name=\'name\']').after('<span style="color:red; margin-left:10px">Lenth must be greater than 3 and less than 128.</span>');
        $('input[name=\'name\']').focus();
        return false;
    } else { $('input[name=\'name\']').css('border', '1px solid #ccc'); $('input[name=\'name\']').next("span").remove();}
    ...
    if(a == '')
    {
        ...
        return false;
    } else { $('input[name=\'code\']').css('border', '1px solid #ccc'); }
    if(a.length < 3 || a.length > 10)
    {
        $('input[name=\'code\']').after('<span style="color:red; margin-left:10px">Lenth must be greater than 3 and less than 10.</span>');
        $('input[name=\'code\']').focus();
        return false;
    } else { $('input[name=\'code\']').css('border', '1px solid #ccc'); $('input[name=\'code\']').next("span").remove(); }
    
    coupanValue = jQuery("#code").val();
    window.opener.$("#coupan").val(coupanValue); 
    $('#form').submit();
    window.close();
}
