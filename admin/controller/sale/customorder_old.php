	...
	private $error = array();
  	...
  	public function getForm() {
		...
		if (isset($this->error['payment_address_1'])) {
			...
			$this->data['error_payment_address_1'] = '';
                ...
                if (isset($this->error['credit_card_type'])) {
      		       $this->data['error_credit_card_type'] = $this->error['credit_card_type'];
    	        } else {
                        $this->data['error_credit_card_type'] = '';
                }
                if ((isset($this->error['credit_card_number'])) ) {
                        $this->data['error_credit_card_number'] = $this->error['credit_card_number'];
                } else {
                        $this->data['error_credit_card_number'] = '';
                }
                
                if (isset($this->error['card_verification_value']) ) {
                        $this->data['error_card_verification_value'] =$this->error['card_verification_value'];
                } else {
                        $this->data['error_card_verification_value'] = '';
                }
		...
		if (isset($this->request->post['customer_id'])) {
			...
			$this->data['custom_payment_method'] = $order_info['custom_payment_method'];
      		...
      		$this->data['custom_payment_method'] = '';
                ...
                if (isset($this->request->post['credit_card_type'])) {
      		$this->data['credit_card_type'] = $this->request->post['credit_card_type'];
		} elseif (!empty($order_info)) { 
			$this->data['credit_card_type'] = $order_info['credit_card_type'];
		} else {
      		$this->data['credit_card_type'] = '';
                }
        
                
                $this->load->helper('encryption');  
                        
                 if (isset($this->request->post['credit_card_number'])) {
                        $this->data['credit_card_number'] = $this->request->post['credit_card_number'];
                        } elseif ((!empty($order_info)) && (!empty($order_info['credit_card_number']))) {
                            $this->data['credit_card_number'] =  Encryptioner::decrypt($order_info['credit_card_number']);
                        } else {
                        $this->data['credit_card_number'] = '';
                }

                if (isset($this->request->post['card_verification_value'])) {
                        $this->data['card_verification_value'] = $this->request->post['card_verification_value'];
                        } elseif ((!empty($order_info)) && (!empty($order_info['card_verification_value']))) {
                                $this->data['card_verification_value'] = Encryptioner::decrypt($order_info['card_verification_value']);
                        } else {
                        $this->data['card_verification_value'] = '';
                }

                if (isset($this->request->post['card_expiry_month'])) {
                        $this->data['card_expiry_month'] = $this->request->post['card_expiry_month'];
                        } elseif (!empty($order_info)) { 
                                $this->data['card_expiry_month'] = $order_info['card_expiry_month'];
                        } else {
                        $this->data['$card_expiry_month'] = '';
                }
                if (isset($this->request->post['card_expiry_year'])) {
                        $this->data['card_expiry_year'] = $this->request->post['card_expiry_year'];
                        } elseif (!empty($order_info)) { 
                                $this->data['card_expiry_year'] = $order_info['card_expiry_year'];
                        } else {
                        $this->data['card_expiry_year'] = '';
                }
  	...
  	protected function validateForm() {
        ...
        if($this->request->post['custom_payment_method'] == 'credit' && ($this->request->post['credit_card_mask'] == -1) ){
            if ((utf8_strlen($this->request->post['credit_card_type']) < 1)) {
                    $this->error['credit_card_type'] = $this->language->get('error_credit_card_type');
            }
            if ((utf8_strlen($this->request->post['credit_card_number']) < 1) ) {
                    $this->error['credit_card_number'] = $this->language->get('error_credit_card_number');;
            }
            if ((utf8_strlen($this->request->post['card_verification_value']) < 1) ) {
	...
	public function info() {
		...
		$order_info = $this->model_sale_customorder->getOrder($order_id);
			...
			$this->data['text_payment_method'] = $this->language->get('text_payment_method');	
                        ...
                        $this->data['text_credit_cvv'] = $this->language->get('text_credit_cvv');
			...
			if ($order_info) {
				...
				$total_data = $this->model_sale_customorder->getOrderTotals($order_id);
                                ...
                                if ($order_info['custom_payment_method'] == 'credit')
                                    $custom_payment_method = 'Credit Card';
                                elseif($order_info['custom_payment_method'] == 'paypal')
                                    $custom_payment_method = 'Paypal';
                                elseif($order_info['custom_payment_method'] == 'affirm')
                                    $custom_payment_method = '12 Months Financing';
				...
				$this->data['orders'][] = array(
					...
					'payment_method'     =>  $custom_payment_method,                   //($order_info['custom_payment_method'] == 'credit') ? 'Credit Card' : 'Paypal',
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$total_data = $this->model_sale_customorder->getOrderTotals($order_id);
					...
					'payment_method'     =>  $custom_payment_method,           //($order_info['custom_payment_method'] == 'credit') ? 'Credit Card' : 'Paypal',
                                        'custom_payment_method'     => $order_info['custom_payment_method'],
                                        'credit_card_type'   => $order_info['credit_card_type'],
                                        'credit_card_number' => (($order_info['credit_card_number']) ?  Encryptioner::decrypt($order_info['credit_card_number']) : '' ),
                                        'card_expiry_month' => $order_info['card_expiry_month'],
                                        'card_expiry_year'  => $order_info['card_expiry_year'],
                                        'firstname'         => $order_info['firstname'],
                                        'lastname'         => $order_info['lastname'],
                                        'card_verification_value'         => (($order_info['card_verification_value']) ? Encryptioner::decrypt($order_info['card_verification_value']) : ''),
					'product'            => $product_data,
					'voucher'            => $voucher_data,
					'total'              => $total_data,
					'comment'            => nl2br($order_info['comment']),
...
/* BOC Print Order CSV */
        public function ordercsv() {
            ...
            if(isset($this->request->post['selected'])) {
                ...
                $results = array();
                $results[0] = array('order_id' => 'Order ID', 'order_item' => 'Ordered Items', 'customer' => 'Customer', 'email' => 'Email', 'status' => 'Status', 'date_added' => 'Date Added', 'total' => 'Total'); 
