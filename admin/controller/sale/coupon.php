...
class ControllerSaleCoupon extends Controller {
    ...
    public function export() {
        ...
        if ($this->request->get['coupon_id']) {
            $results = $this->model_sale_coupon->getCouponHistories($this->request->get['coupon_id']);
            ...
            $results = $this->model_sale_coupon->getAllCouponsHistories();
        ...
        echo "Order ID, Coupon Name, First Name, Last Name, Discounted Amount, Date Added";
        echo "\n";
        arsort($this->data['histories']);
        foreach ($this->data['histories'] as $key => $result) {

            //array_walk($result, 'cleanData');
            // insert data into CSV from here
    ...
    protected function getList() {
        ...
        $this->data['insert_customer_coupon'] = $this->url->link('sale/customerCoupon/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['delete'] = $this->url->link('sale/coupon/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['export'] = $this->url->link('sale/coupon/export', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($results as $result) {
            ...
            $coupon_exclude_categories = array();
            // ISp code Start Here for Exclude Categories
            $exclude_categories = array();
            $exclude_categories = $this->model_sale_coupon->getCouponExcludeCategories($result['coupon_id']);


            $coupon_exclude_category = array();
            foreach ($exclude_categories as $exclude_category_id) {
                $exclude_category_info = $this->model_catalog_category->getCategory($exclude_category_id);

                if ($exclude_category_info) {
                    $coupon_exclude_category[] = ($exclude_category_info['path'] ? $exclude_category_info['path'] . ' &gt; ' : '') . $exclude_category_info['name'];
                }
            }
            // ISp code End Here for Exclude Categories
            ...
            $this->data['coupons'][] = array(
                ...
                'coupon_exclude_categories' => $coupon_exclude_category,
                'action' => $action
            );
        }
        ...
        $this->data['column_date_expire'] = $this->language->get('column_date_expire');
        ...
        $this->data['sort_date_expire'] = HTTPS_SERVER . 'index.php?route=sale/coupon&token=' . $this->session->data['token'] . '&sort=date_expire' . $url;
        ...
        if ($_REQUEST['sendby'] != 'customer') {
            ...
            $this->template = 'sale/coupon_list_customer_cart.tpl';
            $this->children = array(
                'common/header',
                'common/footer'
            );
    ...
    protected function getForm() {
        ...
        $this->data['entry_date_expire'] = $this->language->get('entry_date_expire');
        ...
        if (isset($this->error['date_expire'])) {
            $this->data['error_date_expire'] = $this->error['date_expire'];
        } else {
            $this->data['error_date_expire'] = '';
        }
        ...
        if (!isset($this->request->get['coupon_id'])) {
            ...
            //$this->data['action'] = $this->url->link('sale/customerCoupon/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            ...
            $this->data['export'] = $this->url->link('sale/coupon/export', 'token=' . $this->session->data['token'] . '&coupon_id=' . $this->request->get['coupon_id'] . $url, 'SSL');
            $this->data['history_total'] = $this->model_sale_coupon->getTotalCouponHistories($this->request->get['coupon_id']);
        }
        ...
        // ISp code Start Here for Exclude Categories January01,15
        if (isset($this->request->post['coupon_exclude_category'])) {
            $exclude_categories = $this->request->post['coupon_exclude_category'];
        } elseif (isset($this->request->get['coupon_id'])) {
            $exclude_categories = $this->model_sale_coupon->getCouponExcludeCategories($this->request->get['coupon_id']);
        } else {
            $exclude_categories = array();
        }
        ...
        $this->data['coupon_exclude_category'] = array();
        ...
        foreach ($exclude_categories as $exclude_category_id) {
            ...
            if ($exclude_category_info) {
                $this->data['coupon_exclude_category'][] = array(
                    'category_id' => $exclude_category_info['category_id'],
                    'name' => ($exclude_category_info['path'] ? $exclude_category_info['path'] . ' &gt; ' : '') . $exclude_category_info['name']
                );
            }
        }
        // ISp code End Here for Exclude Categories January01,15
        ...
        if (isset($this->request->post['date_expire'])) {
            $this->data['date_expire'] = $this->request->post['date_expire'];
        } elseif (!empty($coupon_info)) {
            $this->data['date_expire'] = date('Y-m-d', strtotime($coupon_info['date_expire']));
        } else {
            $this->data['date_expire'] = date('Y-m-d', time());
        }
