...
class ModelModuleCustommenu extends Model {
    ...
    public function getParentIds() {
        $query = $this->db->query("SELECT id, name FROM " . DB_PREFIX . "custommenu where parent_id = 0");
        return $query->rows;
    }
    ...
    public function getParentMenuName($id) {
        $query = $this->db->query("SELECT id, name FROM " . DB_PREFIX . "custommenu where id = $id");
        return $query->row;
    }
