...
class ModelSaleOrder extends Model {
	...
	//	$sql = "SELECT o.order_id, email, payment_company, shipping_company, o.customer_id, o.telephone,o.credit_card_number,o.card_verification_value, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.instructions FROM `" . DB_PREFIX . "order` o";
		$sql = "SELECT o.order_id, email, payment_company, shipping_company, o.customer_id, o.telephone,o.credit_card_number,o.card_verification_value, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status,(SELECT value FROM oc_order_payment op WHERE op.order_id = o.order_id) as paymentDetails, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.instructions FROM `" . DB_PREFIX . "order` o";
	...
	public function addOrderHistory($order_id, $data) {
             ...
             $user_id=$this->user->getId();
                ...
                if($data['comment'])
                {
                    $messege = $this->db->escape(strip_tags($data['comment']));
                }
                if($data['msg_template'])
                {
                     
                    $res = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$data['order_status_id'] . "' AND comment_copy_status = 1");
                    if($res->num_rows){
                        //$messege = $this->db->escape(strip_tags($data['msg_template']));
                    }
                }
                $userid_query = $this->db->query("SELECT us.username from " . DB_PREFIX . "user as us where user_id =" . $user_id);
               
                foreach ($userid_query->rows as $userid) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET username = '".$userid['username']."',order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$data['order_status_id'] . "', notify = '" . (isset($data['notify']) ? (int)$data['notify'] : 0) . "', comment = '" .$messege. "', date_added = NOW()");
                }
		$order_info = $this->getOrder($order_id);
