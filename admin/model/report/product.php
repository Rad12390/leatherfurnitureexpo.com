...
class ModelReportProduct extends Model {
	...
	public function getTotalProductsViewed() {
        $query = $this->db->query("SELECT COUNT(*) AS totals FROM " . DB_PREFIX . "product WHERE viewed > 0");
		
		return $query->row['totals'];
	}
	
	public function getTotalProductViews() {
      	$query = $this->db->query("SELECT SUM(viewed) AS totals FROM " . DB_PREFIX . "product");
