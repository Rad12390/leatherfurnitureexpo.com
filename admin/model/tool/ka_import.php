...
class ModelToolKaImport extends Model {
    ...
    protected function saveCategory($product_id, $category_chain,$product_to_category_sort=array()  ,$clear_cache = false) {
//echo "hii"; echo "<br>";
        ...
        $rec = array(
            ...
            'category_id' => $category_id,
            'category_product_sort' => ( ((int) $product_to_category_sort[$category_id]) ?  ((int) $product_to_category_sort[$category_id])  : 99 ),
    ...
    public function findMatches(&$matches, $columns) {
        ...
        $sets = array(
            ...
            'grouped_products' => array('field', 'name', ''),
            'grouped_products_general' => array('field', 'name', ''),
    ...
    protected function getProductId($data) {
        ...
        if (!empty($data['product_id'])) {
            ...
            if (isset($data['model']) && !empty($data['model']) && in_array('model', $this->key_fields)) {
            ...
            if (isset($data['sku']) && !empty($data['sku']) && in_array('sku', $this->key_fields)) {
            ...
            if (isset($data['grouped_sku']) && !empty($data['grouped_sku']) && in_array('sku', $this->key_fields)) {
                if (!empty($this->params['field_lengths']['grouped_sku'])) {
                    if (utf8_strlen($data['sku']) > $this->params['field_lengths']['sku']) {
                        $this->lastError = 'SKU field (' . $data['sku'] . ') exceeds the maximum field size(' .
                                $this->params['field_lengths']['sku'] . "). Product is skipped.";
                    };
                }
                $where[] = "grouped_sku='" . $this->db->escape($data['grouped_sku']) . "'";
            }
...
//        if(!empty($data['name'])){
//             $where[] = "pd.name='" . $this->db->escape($data['name']) . "'";
...
//        }
        ...
        //$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product AS p LEFT JOIN   " . DB_PREFIX . "product_description as pd ON p.product_id = pd.product_id
    ...
    protected function updateProduct($product_id, $data, $is_new) {
        ...
        if (isset($data['tax_class'])) {
           ...
           $tax_class_id =  $this->config->has('aes_default_product_tax_class_id') ? $this->config->get('aes_default_product_tax_class_id') : 0;
           $product['tax_class_id'] =  $tax_class_id;
        ...
        if (isset($data['name_for_cateogory']))
            $lang['name_for_cateogory'] = trim($data['name_for_cateogory']);
       ...
       if (isset($data['date_available'])) {
            if (!$this->formatDate($data['date_available'])) {
                ...
                if ($data['date_available'] != '0000-00-00' && $data['date_available'] != '') {
    ...
    protected function saveCategories($product_id, $data, $delete_old, $is_new) {
        ...
        if (empty($data['category_id']) && empty($data['category'])) { //echo "<pre>"; print_r($data);
            return false;
        }
            ...
            foreach ($cats_list as $cat) {
                ...
                if (!empty($qry->row)) {
                    $rec = array(
                        ...
                        'category_product_sort' => ( ((int) $product_to_category_sort[$qry->row['category_id']]) ?  ((int) $product_to_category_sort[$qry->row['category_id']])  : 99 ),
                    );
                    $this->kadb->queryInsert('product_to_category', $rec, true);
        ...
        if (!empty($data['category']) && empty($data['category_id'])) {
			...
			if (!empty($data['category'])) {
                ...
                foreach ($cats_list as $cat) {
                    if ($this->saveCategory($product_id, $cat,$product_to_category_sort)) {              //pass previous stored product to category sort order
                        $category_assigned = true;
                    }
                }
            }
...
//				AND name='" . $this->db->escape($option['value']) . "'");
            ...
            $qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "option ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_id = ovd.option_id) WHERE ((ov.type = 'select')||(ov.type = 'image')) AND ovd.name='" . $this->db->escape($option['value']) . "' AND ovd.language_id='" . $this->params['language_id'] . "'");
            ...
            if (isset($option['gradeforcolor'])) {

                $rec['gradeforcolor'] = "[" . json_encode($this->getgradeforcolor($option['gradeforcolor'])) . "]";
            }
            if (isset($option['gradeforcolor'])) {

                $rec['option_child_id'] = $this->getOptsManufacturer($option['manufacturer']);
            }
    ...
    public function saveGroupedProductsGeneral($row, $product, $delete_old, $is_first_product) {
        if ($is_first_product) {
            if (!empty($this->params['matches']['grouped_products_general'])) {
                $gp_data_general = array();
                foreach ($this->params['matches']['grouped_products_general'] as $ck => $cv) {
                    if (empty($cv['column']))
                        continue;

                    $val = $row[$cv['column'] - 1];
                    $gp_data_general[$cv['field']] = trim($val);
                }

                $query_get = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_grouped_type WHERE product_id = '" . (int) $product['product_id'] . "'");
                if ($query_get->row) {

                    $this->db->query("UPDATE " . DB_PREFIX . "product_grouped_type SET product_type='" . $gp_data_general['gp_product_type'] . "' WHERE product_id = '" . (int) $product['product_id'] . "'");
                } else {

                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_grouped_type SET product_type='" . $gp_data_general['gp_product_type'] . "', product_id = '" . (int) $product['product_id'] . "'");
                }


                /// Update Ctaegory Product info and Call for price , starting_price_product

                $this->db->query("UPDATE " . DB_PREFIX . "product SET product_info='" . $gp_data_general['gp_category_product_info'] . "',call_for_price='" . $gp_data_general['gp_call_for_price_product'] . "',starting_price_product='" . $gp_data_general['starting_price_product'] . "'  WHERE  product_id = '" . (int) $product['product_id'] . "'");

                /// delete and insert dicounts
                $this->db->query("DELETE  FROM " . DB_PREFIX . "product_grouped_discount WHERE product_id = '" . (int) $product['product_id'] . "'");
                $this->db->query("INSERT  INTO " . DB_PREFIX . "product_grouped_discount SET product_id = '" . (int) $product['product_id'] . "',discount='" . $gp_data_general['gp_discount'] . "',type='" . $gp_data_general['gp_discount_type'] . "'");
            }
        }
    }

    public function saveGroupedProducts($row, $product, $delete_old) {

        if (!empty($this->params['matches']['grouped_products'])) {
            $gp_data = array();
            foreach ($this->params['matches']['grouped_products'] as $ck => $cv) {
                if (empty($cv['column']))
                    continue;

                $val = $row[$cv['column'] - 1];
                $gp_data[$cv['field']] = trim($val);
            }
            // echo "<pre>"; print_r($gp_data);
            if (!empty($gp_data['sku_gp'])) {
                $array_grade_price_group = array();
                $array_grade_price = array();
                if (!empty($gp_data['grade_price_group'])) {
                    $array_grade_price_group = explode($this->params['cat_separator'], $gp_data['grade_price_group']);
                    $gp_data['grade_price_group'] = $array_grade_price_group;
                }
                if (!empty($gp_data['grade_price'])) {
                    $array_grade_price = explode($this->params['cat_separator'], $gp_data['grade_price']);
                    $gp_data['grade_price'] = $array_grade_price;
                }
                $this->saveGroupedProduct($gp_data, $product, $delete_old);
            } else {
    ...
    public function saveGroupedProduct($gp_data, $product, $delete_old) {
//echo "<pre>"; print_r($gp_data); exit;


        if (!empty($this->params['matches']['grouped_products'])) {
            if ($delete_old) {
                $delete_gp_products = "DELETE FROM " . DB_PREFIX . "product_grouped WHERE product_id = '" . (int) $product['product_id'] . "'";
                $this->db->query($delete_gp_products);
            }
        }

        // get grouped product id 

        $get_gp_product_id_sql = "select product_id from " . DB_PREFIX . "product where sku = '" . $gp_data['sku_gp'] . "'";
        $result_p_id = $this->db->query($get_gp_product_id_sql);
        $gp_product_id = 0;
        if (empty($result_p_id->row)) {
            $this->addImportMessage("Grouped product not found sku_gp - " . $gp_data['sku_gp']);
        } else {
            $gp_product_id = $result_p_id->row['product_id'];
        }
        // check if grouped product already attached with main product
        if ($gp_product_id) {

            // update heigth,width,depth
            //  $update_hwd_sql = "UPDATE " . DB_PREFIX . "product SET width='".$gp_data['gp_depth']."', height='".$gp_data['gp_height']."',length='".$gp_data['gp_width']."' WHERE product_id ='" . (int) $gp_product_id . "'";
            // $this->db->query($update_hwd_sql);
            // delete product grade and price we will insert in update and insert 

            $delete_sql = "DELETE FROM " . DB_PREFIX . "grade_price_product WHERE grouped_product_id ='" . (int) $gp_product_id . "' AND 	product_id ='" . (int) $product['product_id'] . "'";
            $this->db->query($delete_sql);

            $check_sql = "select product_grouped_id from " . DB_PREFIX . "product_grouped WHERE product_id = '" . (int) $product['product_id'] . "' AND grouped_id = '" . (int) $gp_product_id . "'";
            $result_check_sql = $this->db->query($check_sql);
            if (empty($result_check_sql->row)) {

                $product_grouped_id = 0;
            } else {
                $product_grouped_id = $result_check_sql->row['product_grouped_id'];
            }
            if ($product_grouped_id) {
                // update grouped product
                if (empty($gp_data['is_starting_price'])) {
                    $gp_data['is_starting_price'] = 0;
                }
                $update_gp_sql = "UPDATE " . DB_PREFIX . "product_grouped SET  is_starting_price = '" . $gp_data['is_starting_price'] . "', image = '" . $gp_data['gp_image'] . "', grouped_maximum = '" . (int) $gp_data['gp_qty'] . "', product_price = '" . (int) $gp_data['grouped_product_price'] . "', gp_width='" . $gp_data['gp_width'] . "', gp_height='" . $gp_data['gp_height'] . "',gp_depth='" . $gp_data['gp_depth'] . "' WHERE product_grouped_id='" . $product_grouped_id . "'";
                $this->db->query($update_gp_sql);

                /// grade option id
                if (!empty($gp_data['grade_price_group'])) {
                    foreach ($gp_data['grade_price_group'] as $key => $option_name) {
                        $grade_option_data = $this->getgradeOptionValues($option_name);

                        if (empty($grade_option_data)) {
                            $this->addImportMessage("Grade Option value not found for Option name  - " . $option_name);
                        } else {
                            $grade_option_value_id = $grade_option_data['option_value_id'];
                            $grade_price = (isset($gp_data['grade_price'][$key]) ? $gp_data['grade_price'][$key] : 0);
                            $insert_sql = "INSERT INTO " . DB_PREFIX . "grade_price_product SET grouped_product_id ='" . (int) $gp_product_id . "', 	product_id ='" . (int) $product['product_id'] . "',grade_option_value_id='" . (int) $grade_option_value_id . "',grade_price='" . (int) $grade_price . "'";
                ...
                // Insert new grouped product

                if (empty($gp_data['is_starting_price'])) {
                    $gp_data['is_starting_price'] = 0;
                }

                $insert_gp_sql = "INSERT INTO " . DB_PREFIX . "product_grouped SET   product_id = '" . (int) $product['product_id'] . "', grouped_id = '" . (int) $gp_product_id . "' , is_starting_price = '" . $gp_data['is_starting_price'] . "', image = '" . $gp_data['gp_image'] . "', grouped_maximum = '" . (int) $gp_data['gp_qty'] . "', product_price = '" . (int) $gp_data['grouped_product_price'] . "', gp_width='" . $gp_data['gp_width'] . "', gp_height='" . $gp_data['gp_height'] . "',gp_depth='" . $gp_data['gp_depth'] . "'";
                $this->db->query($insert_gp_sql);
    ...
    public function getgradeOptionValues($option_value_name) {

        $option_value_data = array();

        $option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE ov.option_id = '32' AND ovd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND ovd.name='" . $option_value_name . "' ORDER BY ovd.name ASC");
...
//###################################################################################################################
                ...
                // This code is the default to process common data as first line 
                // Tarsem comment this because client want to sort the CSV that missed up all system have no info which is first line
...
//#################################################################################################################
                ...
                if (isset($data['main_line'])) {
                    ...
                    $this->addImportMessage("main_line column missing");
                }
                ...
                $this->saveGroupedProducts($row, $product, $delete_old);
                $this->saveGroupedProductsGeneral($row, $product, $delete_old, $is_first_product);
    ...
    protected function addCustomProductFields(&$fields) {
        $default_fields = array('product_id', 'model', 'sku', 'grouped_sku', 'grouped_product_price', 'starting_price_product', 'product_info', 'call_for_price', 'upc', 'ean', 'jan', 'isbn', 'mpn',
    ...
    public function getFieldSets() {
        ...
        $ext_options = array(
            ...
            'gradeforcolor' => array(
                'field' => 'gradeforcolor',
                'name' => 'Select A Color - Grade',
                'descr' => ''
            ),
            'manufacturer' => array(
                'field' => 'manufacturer',
                'name' => 'Manufacturer',
                'descr' => 'This field is used for Manufacturer on Options Tab of Grouped product screen'
            ),
        );
        ...
        $grouped_products = array(
            'sku_gp' => array(
                ...
                'descr' => 'This is just for mapping it will not imported in DB',
            ),
            'gp_qty' => array(
                'field' => 'gp_qty',
                'name' => 'gp_qty',
            ...
            'is_starting_price' => array(
                ...
                'descr' => 'Starting Price',
            ),
            'grade_price_group' => array(
                'field' => 'grade_price_group',
                'name' => 'grade_price_group',
                'descr' => 'Grade price group',
            ),
            'grade_price' => array(
                'field' => 'grade_price',
                'name' => 'grade_price',
        ...
        $grouped_products_general = array(
        ...
        $sets = array(
            ...
            'grouped_products_general' => $grouped_products_general,
        );
    ...
    public function getgradeforcolor($name) {
        $qry = $this->db->query("SELECT option_value_id FROM " . DB_PREFIX . "option_value_description WHERE 
				option_id = '32'
				AND language_id = '" . $this->params['language_id'] . "'
				AND name='" . $this->db->escape($name) . "'");
        if ($qry->row) {
            return $qry->row['option_value_id'];
        } else {
            return '';
        }
    }

    public function getOptsManufacturer($m_name) {
        if (!empty($m_name)) {
            $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_description` od LEFT JOIN `" . DB_PREFIX . "option` o ON (o.option_id = od.option_id) WHERE o.type = 'image' AND od.language_id = '" . (int) $this->config->get('config_language_id') . "' AND od.name='" . $m_name . "'");
