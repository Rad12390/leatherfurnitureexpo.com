...
class ModelCatalogSEOEditor extends Model {
	...
	public function gettotalSEOs($data = array()) {
			...
			$sql.="	 
				...
				select 'GroupedProduct' as type, p.product_id  from " . DB_PREFIX . "product p
				  inner join " . DB_PREFIX . "product_description pd on p.product_id = pd.product_id
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('product_id=', CAST(p.product_id as CHAR)) = u.query
				  
				  where pd.language_id = '" . (int)$this->session->data['language_id'] . "' AND model='grouped' ";
			if (!empty($data['filter_name'])) {
				$sql .= " AND LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
			}
	...
	public function getSEOs($data = array()) {
			...
			//ajay
				...
				select 'GroupedProduct' as type, pd.name as name, p.product_id as id, u.keyword, pd.meta_description, pd.meta_keyword, pd.tag AS tags from " . DB_PREFIX . "product p
				  inner join " . DB_PREFIX . "product_description pd on p.product_id = pd.product_id
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('product_id=', CAST(p.product_id as CHAR)) = u.query
				  
				  
				  where pd.language_id = '" . (int)$this->session->data['language_id'] . "' AND p.model = 'grouped' ";
			if (!empty($data['filter_name'])) {
				$sql .= " AND LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
			}
