...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "', status = '" . $option_value['status'] . "'");
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "', status = '" .$option_value['status'] . "'");
				} else {
	...
	public function getGradepriceOptionValues() {
		$option_value_data = array();
		
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value_description WHERE option_id = '32' ORDER BY name ASC");
	...
	public function getOptionValuesforselectedcolor($option_id) {
		$option_value_data = array();
		
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE ov.option_id = '" . (int)$option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order ASC");
		//echo "<pre>";
		//print_r($option_value_query);
	...
	public function getgradeOptionValues($option_id) {
		$option_value_data = array();
		
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE ov.option_id = '32' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ovd.name ASC");
	...
	public function getallOptionValues($option_id) {
		$option_value_data = array();
		
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_id = ovd.option_id) WHERE ov.type = 'image' ORDER BY ov.sort_order ASC");
	...
	public function getselctcolorOptionValues($option_id) {
		$option_value_data = array();
		
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option ov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_id = ovd.option_id) WHERE ((ov.type = 'select')||(ov.type = 'image')) ORDER BY ov.sort_order ASC");
	...
	public function getOptionValueDescriptions($option_id) {
		...
		//$a =	array_multisort($option_value_data);
		//print_r($a);
...
public function getOpsValues($option_id) {
		$option_value_data  = array();
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option_description` od LEFT JOIN `" . DB_PREFIX . "option` o ON (o.option_id = od.option_id) WHERE o.type = 'image' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		
		return $query->rows;
	}
	
	  public function getOpslect() {
