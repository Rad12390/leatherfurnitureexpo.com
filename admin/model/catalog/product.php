...
class ModelCatalogProduct extends Model {
	...
	public function getProductsByCategoryId($category_id) {
		...
		$query = $this->db->query("SELECT distinct p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id IN(" . $category_id . ") ORDER BY pd.name ASC");
								  
		return $query->rows;
	} 
         public function getTotalProductIdByCategoryId($category_id) {
          
		$query = $this->db->query("SELECT distinct count(p.product_id) FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id IN(" . $category_id . ") ORDER BY pd.name ASC");
