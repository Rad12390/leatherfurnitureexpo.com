...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `display_overlay_image_on_category_page` = '" .(isset($data['display_overlay_image_on_category_page']) ? (int)$data['display_overlay_image_on_category_page'] : 0 ). "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editCategory($category_id, $data, $result) {
	
			
		foreach ($result as $productid) {

		 $sort= 'cat_sort'. $productid['product_id'];
		 $prod_id= 'product_id'. $productid['product_id'];

	   	$this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET category_product_sort = '" . (int)$data[$sort] . "' WHERE category_id = '" . (int)$category_id . "' AND 	product_id = '" . (int)$data[$prod_id] . "' ");

	}
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "',`display_overlay_image_on_category_page` = '" .(isset($data['display_overlay_image_on_category_page']) ? (int)$data['display_overlay_image_on_category_page'] : 0 ). "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");


		$this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET category_product_sort = '" . (int)$data['cat_sort'] . "' WHERE category_id = '" . (int)$category_id . "' AND 	product_id = '" . (int)$data['product_id'] . "' ");
